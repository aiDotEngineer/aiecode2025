...
import { frontCopy, backCopy } from "~/app/frontCopy";
...
function formatDerivedTracksAndSessionsToText(
  groupedSessions: Map<string, SessionWithSpeakerInfo[]>
): string {
  let text = "";
...
export function formatLlmsFullTxt() {
  ...
  allSpeakers.forEach((speaker) => {
    speaker.attributes.sessions.data.forEach((sessionAttr) => {
      ...
      const trackNames = trackDataName
        .split(",")
        .map((tn) => tn.trim())
        .filter((tn) => tn);
