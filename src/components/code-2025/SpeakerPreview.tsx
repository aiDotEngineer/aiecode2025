...
// import { useSummit2025 } from '../ChoosePrimaryLayout';
...
export function SpeakerPreview({ presenters, tracks = [] }: Props) {
  ...
  // const { SUBMIT_TALK_URL } = useSummit2025();
  ...
  return (
    <div className="space-y-8">
      ...
      {/* <p className="text-center space-y-6 max-w-3xl m-auto text-gray-on-white">
        Here are our speakers at a glance!
      </p> */}
      ...
      {textView ? (
        ...
        <ul
          role="list"
          className="max-w-6xl m-auto grid grid-cols-2 sm:grid-cols-4 gap-y-1 sm:gap-x-4 lg:grid-cols-6 items-baseline text-sm pb-16"
        >
        ...
        <ul
          role="list"
          className={
            'grid grid-cols-3 gap-1 sm:grid-cols-4 lg:gap-2 lg:grid-cols-8 xl:-mx-32 pb-16'
          }
        >
          {presentersProcessed.map((presenter) => {
            const { name, socialLinks, sessions, company } =
            ...
            return (
              <li key={presenter.id}>
                <div className="group relative overflow-hidden rounded-lg lg:rounded-2xl bg-neutral-100">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    alt={name}
                    loading="lazy"
                    src={url}
                    className="h-64 lg:h-48 w-full object-cover transition duration-500 motion-safe:group-hover:scale-105"
                  />
